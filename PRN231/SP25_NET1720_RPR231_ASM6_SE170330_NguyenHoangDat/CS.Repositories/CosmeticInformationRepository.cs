...
namespace CS.Repositories
{
    public class CosmeticInformationRepository : GenericRepository<CosmeticInformation>
    {
        ...
        public async Task<CosmeticInformation> GetByIdAsync(string id) => await _context.CosmeticInformations.Include(item => item.Category).FirstOrDefaultAsync(item => item.CosmeticId == id);
